chrome.storage.local.get({ breachWarning: 'defaultValue' }, (items) => {
  if (items.breachWarning) {
    ...
    document.getElementById('noBreachMessage').innerHTML =
      "<p>This does not mean this site has never been breached, only that no information regarding a breach of this site has been made public at this time. You should still be sure to <span id='never'>never</span> use the same password for multiple services.</p>";
  }
});
